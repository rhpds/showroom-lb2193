...
NOTE: The type can be one of job_template, project, inventory_source,
workflow_approval.
...
WARNING: It still requires a valid unified job template, but it will delete the
...
* https://github.com/redhat-cop/controller_configuration/tree/devel/roles/workflow_job_templates[workflow_job_templates role]
...
[source,console]
----
ansible-playbook -i inventory.yml -l control playbooks/export_workflow.yml
----

NOTE: Notice that the exported workflow has a lot more meta information that
