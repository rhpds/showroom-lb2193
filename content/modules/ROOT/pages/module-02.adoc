...
to include a new EE (that we will show how to create with code in the next module) called `config_as_code` with image path `{{ ah_host }}/config_as_code` that is pull `always` and uses the credential `cr_ah`.
